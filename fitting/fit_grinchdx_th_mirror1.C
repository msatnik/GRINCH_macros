...
void fit_grinchdx_th_mirror1(){ 
  ...
  // Load Histograms
  TH2D *origHist = (TH2D*)f1->Get("h_BBgr_clusxdiff_trth_allclus_mirror1");
  TH2D *origHist2 = (TH2D*)f2->Get("h_BBgr_clusxdiff_trth_allclus_mirror1");

  TH2D* combinedhist = (TH2D*)(origHist->Clone("combinedhist"));
  combinedhist ->Add(origHist2);
    ...
    graph = FitBinsGausTH2D(combinedhist);
    // customize graph
    ...
    fit_linear ->SetParameters(0.15, 1.3);
   ...
   // Add entries to the legend for the fit parameters
    ...
    legend->AddEntry("", Form("offset = %.4f +- %.4f ", linear_offset, linear_offset_error), "");
    ...
    // fit_trad ->SetParameters(-0.07,0.03);
  ...
  for (int i = 0; i<nBins; i++) // 
    {
      ...
      if( (binCenter > -0.24 && binCenter < -0.18) || (binCenter > -0.15 && binCenter < -0.12)){
	...
	double mean_prediction = 1.3*binCenter + 0.16;
