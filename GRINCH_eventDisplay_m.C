...
void GRINCH_eventDisplay_m()//main
{
...
{  
  ...
  while (!clusterflag){
	...
	sum_array[pmtcntr] =  Sum_Adjacent_Hits(pmtcntr);
  ...
  // cout<<"stack of interesting PMTs"<<endl;
  ...
  /*
    ...
    test = copystack.top();
    copystack.pop();
    //cout<< "before Test_Reference: "<< x << ", "<<y<<endl;
    Calculate_PMT_Coord(x, y, test);
    //Test_Reference(x,y,test);
    cout<< "PMT "<<test << " is at "<< x << ", "<<y<<endl;
    //cout<<"PMT "<<PMT<<" is at x = "<< x << " y = "<<y<<endl;
    }
  */
  
  Calculate_Cluster_Center(cluster_members_stack,clusterx,clustery);
  cout<<"cluster center is at "<<clusterx<<", "<<clustery<<endl;
  
  Int_t vertspread = 0;
  Int_t horizspread = 0; 

  Find_Cluster_Extrema(cluster_members_stack,horizspread,vertspread);
...
{ 
  ...
  cluster_members_stack = already_counted;
