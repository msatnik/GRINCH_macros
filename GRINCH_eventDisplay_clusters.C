...
TH2D  *h_BBgr_hit_amp_elemID = new TH2D("h_BBgr_hit_amp_elemID", "ToT vs PMT for each PMT; PMT number; ToT", 510,0,510, 50,0,50 );
TH2D *h_BBgr_hit_xhit_elemID = new TH2D("h_BBgr_hit_xhit_elemID", "X vs PMT num ;PMT num; xhit",510,0,510,100,-1,1);
TH2D *h_BBgr_hit_yhit_elemID = new TH2D("h_BBgr_hit_yhit_elemID", "Y vs PMT num ;PMT num; yhit",510,0,510,100,-0.25,0.25);
 TH2D  *h_BBgr_hit_clusindex_elemID = new TH2D("h_BBgr_hit_clusindex_elemID", "clusindex vs PMT for each PMT; PMT number; clusindex", 510,0,510, 21,-1,20 );



TH2D *h_BBgr_allclus_trackindex_clusindex = new TH2D("h_BBgr_allclus_trackindex_clusindex","track index vs cluster number;  cluster index; track idex",20,0,20,11,-1,10);
TH2D *h_BBgr_allclus_adc_clusindex = new TH2D("h_BBgr_allclus_adc_clusindex","ToT sum vs cluster number; cluster index; ToT sum",20,0,20,200,0,200);
TH2D *h_BBgr_allclus_size_clusindex = new TH2D("h_BBgr_allclus_size_clusindex","clus size vs clus index; cluster index;clus size", 20,0,20,20,0,20);
TH2D *h_BBgr_allclus_tmean_clusindex = new TH2D("h_BBgr_allclus_tmean_clusindex","tmean vs cluster index; cluster index;tmean",20,0,20,60,-30,30);
TH2D *h_BBgr_allclus_ymean_clusindex = new TH2D("h_BBgr_allclus_ymean_clusindex","ymean vs cluster index; cluster index; ymean",20,0,20,100,-0.25,0.25 );
TH2D *h_BBgr_allclus_xmean_clusindex = new TH2D("h_BBgr_allclus_xmean_clusindex","xmean vs cluster index; cluster index;xmean",20,0,20,100,-1,1);
TH2D *h_BBgr_allclus_dx_clusindex = new TH2D("h_BBgr_allclus_dx_clusindex","dx vs cluster index; cluster index; dx",20,0,20,100,-1,1); 
TH2D *h_BBgr_allclus_dy_clusindex = new TH2D("h_BBgr_allclus_dy_clusindex","dy vs cluster index; cluster index; dy",20,0,20,100,-1,1); 
...
TH1D* h_BBgr_clus_trackindex =  new TH1D("h_BBgr_clus_trackindex", ";bb.grinch_tdc.clus.trackindex;",5,-1,4);
  TH1D* h_BBgr_clus_xmean =  new TH1D("h_BBgr_clus_xmean", ";bb.grinch_tdc.clus.xmean;",100,-1,1);
  TH1D* h_BBgr_clus_ymean =  new TH1D("h_BBgr_clus_ymean", ";bb.grinch_tdc.clus.ymean;",100,-0.25,0.25);
TH1D* h_BBgr_clus_mirrorindex =  new TH1D("h_BBgr_clus_mirrorindex", ";bb.grinch_tdc.clus.mirrorindex;",100,-0.25,0.25);
...
{  
 ...
 cClus->Divide(2,2);
  ...
  // while(!eventflag){
    ...
    h_BBgr_hit_amp_elemID ->Reset();
    h_BBgr_hit_xhit_elemID ->Reset();
    h_BBgr_hit_yhit_elemID ->Reset();
    h_BBgr_hit_clusindex_elemID ->Reset();

    h_BBgr_allclus_trackindex_clusindex ->Reset();
    h_BBgr_allclus_adc_clusindex ->Reset();
    h_BBgr_allclus_size_clusindex ->Reset();
    h_BBgr_allclus_tmean_clusindex ->Reset();
    h_BBgr_allclus_ymean_clusindex ->Reset();
    h_BBgr_allclus_xmean_clusindex ->Reset();
    h_BBgr_allclus_dx_clusindex->Reset();
    h_BBgr_allclus_dy_clusindex->Reset();


    h_BBgr_clus_trackindex  ->Reset();
    h_BBgr_clus_adc ->Reset();
    h_BBgr_clus_size ->Reset();
    h_BBgr_clus_tmean ->Reset();
    h_BBgr_clus_ymean ->Reset();
    h_BBgr_clus_xmean ->Reset();
    h_BBgr_clus_mirrorindex ->Reset();


    Int_t bestcluster = BBgr_bestcluster;
    ...
    for (Int_t i = 0; i < hitsGR ; i++){
      ...
      h_BBgr_hit_xhit_elemID ->Fill(pmtnum, BBgr_hit_xhit[i]);
      //cout<<"xhit "<<BBgr_hit_xhit[i] <<endl;
      h_BBgr_hit_yhit_elemID ->Fill(pmtnum, BBgr_hit_yhit[i]);
      h_BBgr_hit_clusindex_elemID ->Fill(pmtnum, BBgr_hit_clustindex[i]);

      Signal_Label[pmtnum]->SetTextColor(kRed);
      PMT[pmtnum]->SetFillColor(kRed);
      PMT_Label[pmtnum]->SetFillColor(kRed);

      if (BBgr_hit_clustindex[i] == bestcluster)
	{
	  Signal_Label[pmtnum]->SetTextColor(kGreen);
	  PMT[pmtnum]->SetFillColor(kGreen);
	  PMT_Label[pmtnum]->SetFillColor(kGreen);
	}
    ...
    for (Int_t i = 0; i < Nclusters; i++)
      {
	h_BBgr_allclus_trackindex_clusindex->Fill(i,BBgr_allclus_trackindex[i]);  
	h_BBgr_allclus_adc_clusindex ->Fill(i, BBgr_allclus_adc[i]);
	h_BBgr_allclus_size_clusindex ->Fill(i, BBgr_allclus_size[i]);
	h_BBgr_allclus_tmean_clusindex ->Fill(i, BBgr_allclus_tmean[i] - HODOtmean[0]);  
	h_BBgr_allclus_ymean_clusindex ->Fill(i, BBgr_allclus_ymean[i]);
	h_BBgr_allclus_xmean_clusindex ->Fill(i, BBgr_allclus_xmean[i]);
	h_BBgr_allclus_dx_clusindex ->Fill(i, BBgr_allclus_dx[i]);
	h_BBgr_allclus_dy_clusindex->Fill(i,BBgr_allclus_dy[i]);
      }
  ...
  allC->cd(3);
  h_BBgr_hit_xhit_elemID ->Draw("COLZ");
  allC->cd(4);
  h_BBgr_hit_clusindex_elemID ->Draw("colz");
  ...
  cAllClus->cd(1);
  h_BBgr_allclus_size_clusindex ->Draw("COLZ");
  cAllClus->cd(2);
  h_BBgr_allclus_tmean_clusindex ->Draw("colz");
  cAllClus->cd(3);
  h_BBgr_allclus_xmean_clusindex ->Draw("colz");
  cAllClus->cd(4);
  h_BBgr_allclus_trackindex_clusindex ->Draw("colz");

  cAllClus -> SetTitle(Form("allclus: Event %d",event_cnt));


  cClus->cd(1);
  h_BBgr_clus_size ->Draw("colz");
  cClus->cd(2);
  h_BBgr_clus_tmean ->Draw("colz");
  cClus->cd(3);
  h_BBgr_clus_xmean ->Draw("colz");
  cClus->cd(4);
  h_BBgr_clus_trackindex ->Draw("colz");

  cClus -> SetTitle(Form("clus: Event %d",event_cnt));
  ...
  cAllClus->Modified(); allC->Update(); gSystem->ProcessEvents();
  cClus->Modified(); allC->Update(); gSystem->ProcessEvents();
